...
static int aw8624_ram_init(struct aw8624 *aw8624)
{
	...
	queue_delayed_work(aw8624->work_queue, &aw8624->ram_work,  msecs_to_jiffies(ram_timer_val));
#else
	aw8624_ram_update(aw8624);
#endif
	return 0;
...
static int aw8624_haptic_rtp_init(struct aw8624 *aw8624)
{
	...
	if (aw8624->play_mode == AW8624_HAPTIC_RTP_MODE && !atomic_read(&aw8624->exit_in_rtp_loop)) {
		aw8624_haptic_set_rtp_aei(aw8624, true);
	}
					 ...
					 unsigned char flag)
{
	...
	if (flag) {
		...
		if (aw8624->activate_mode == AW8624_HAPTIC_ACTIVATE_RAM_LOOP_MODE) {
			aw8624_haptic_set_repeat_wav_seq(aw8624, (aw8624->info.effect_id_boundary + 1));
			pr_debug("%s  line=%d\n", __func__, __LINE__);
...
static void aw8624_rtp_work_routine(struct work_struct *work)
{
	...
	if (aw8624->state) {
		...
		if (aw8624->rtp_file_num > ((sizeof(aw8624_rtp_name) / AW8624_RTP_NAME_MAX) - 1))
			aw8624->rtp_file_num = (sizeof(aw8624_rtp_name) / AW8624_RTP_NAME_MAX) - 1;
...
static int aw8624_haptic_init(struct aw8624 *aw8624)
{
	...
	/* vbat compensation */
	aw8624_haptic_cont_vbat_mode(aw8624, AW8624_HAPTIC_CONT_VBAT_HW_COMP_MODE);
...
static irqreturn_t aw8624_irq(int irq, void *data)
{
	...
	if (reg_val & AW8624_BIT_SYSINT_FF_AEI) {
		...
		if (aw8624->rtp_init) {
			while ((!aw8624_haptic_rtp_get_fifo_afi(aw8624)) &&
			       (aw8624->play_mode == AW8624_HAPTIC_RTP_MODE)  && !atomic_read(&aw8624->exit_in_rtp_loop)) {
				mutex_lock(&aw8624->rtp_lock);
				pr_debug
			   ...
			   struct device_node *np)
{
	...
	val =
	    of_property_read_u32(np, "vib_gain_flag",
				 &aw8624->info.gain_flag);
	if (val != 0)
		printk("%s vib_gain_flag not found\n", __func__);
	val =
	...
	printk("%s ---%d aw8624->effects_count=%d\n", __func__, __LINE__, aw8624->effects_count);
	...
	printk(" 20190420_dt       aw8624->effects_count: %d\n",
	       aw8624->effects_count);
	printk(" 20190420_dt       aw8624->effect_id_boundary: %d\n",
	       aw8624->info.effect_id_boundary);
	printk(" 20190420_dt       aw8624->effect_max: %d\n",
	       aw8624->info.effect_max);
	printk(" 20190420_dt       aw8624->info.cont_drv_lvl: %d\n",
		   aw8624->info.cont_drv_lvl);
	printk(" 20190420_dt       aw8624->info.cont_drv_lvl_ov: %d\n",
		   aw8624->info.cont_drv_lvl_ov);
	printk(" 20190420_dt       aw8624->info.gain_flag: %d\n",
		   aw8624->info.gain_flag);
	for (i = 0; i < 3; i++)
		for (j = 0; j < 5; j++)
			printk(" 20190420_dt       aw8624->info.trig_config[%d][%d]: %d\n",
				i, j, aw8624->info.trig_config[i][j]);
	for (i = 0; i < 175; i++)
		printk(" 20190420_dt       aw8624->info.rtp_time[%d]: %d\n",
		   i, aw8624->info.rtp_time[i]);
	printk(" 20190420_dt       aw8624->info.parameter1: 0x%x\n",
	       aw8624->info.parameter1);
	return 0;
}
					...
					struct ff_effect *old)
{
	...
	if (aw8624->effect_type == FF_CONSTANT) {
		...
		pr_debug("%s 20191202_aw8624->effect_id =%d \n", __func__, aw8624->effect_id);
		...
		if (aw8624->effect_id < aw8624->info.effect_id_boundary) {
			...
			pr_debug("%s aw8624->predefined[aw8624->effect_id].play_rate_us/1000 = %d\n",
				__func__, aw8624->predefined[aw8624->effect_id].play_rate_us/1000);
		}
		if (aw8624->effect_id >= aw8624->info.effect_id_boundary) {
...
{
	struct aw8624 *aw8624 = container_of(work, struct aw8624, set_gain_work);
...
static DEVICE_ATTR(osc_save, S_IWUSR | S_IRUGO, aw8624_osc_cali_show, aw8624_osc_save_store);
 ...
 ******************************************************/
static int
aw8624_i2c_probe(struct i2c_client *i2c,
			    const struct i2c_device_id *id)
{
	...
	if (!aw8624->work_queue) {
		dev_err(&i2c->dev, "%s: Error creating aw8624_vibrator_work_queue\n",
			__func__);
		goto err_sysfs;
	}
	INIT_WORK(&aw8624->set_gain_work, set_gain);
